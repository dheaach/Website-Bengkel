...
<div class="row">
	<div class="col-md-12">
		...
		<center><h1 style="font-family: Rockwell Condensed;font-size: 56px;">Penjualan</h1></center>
		<a href="<?php echo base_url('penjualan/penjualan_langsung') ?>" class="btn btn-primary">Penjualan Langsung</a>	
		<a href="<?php echo base_url('penjualan1/penjualan_servis') ?>" class="btn btn-primary">Penjualan Servis</a>
	</div>
	...
	<div class="col-md-12">
		<table border="1" cellpadding="5" style="margin-bottom: 10px;margin-top: 10px;" id="example" class="table table-bordered">
			...
			<tbody>
				<?php 
					$sql = $this->db->query("SELECT * FROM customer INNER JOIN karyawan INNER JOIN penjualan ON penjualan.idcust = customer.id AND karyawan.id = penjualan.staff ORDER BY jenis DESC")->result();
					$no = 1;
					foreach ($sql as $row) {
				?>
				<tr>
